...
class Class extends React.Component {
  constructor(props) {
    ...
    if (location.search || state.__back) {
      dispatch({type: 'fundAccount/setTabCondition', payload: location.search, index: 2});
    } else {
      //不是后退 重置条件参数值
      dispatch({type: 'fundAccount/resetTabCondition', index: 2});
    }
  ...
  handleExport() {
    ...
    dispatch({
      type: 'fundAccount/companyExport',
      payload: {},
      callback(res) {
  ...
  onSearch() {
    const {dispatch} = this.props;
    return dispatch({type: 'fundAccount/getTab2Data', payload: {}});
  }

  render() {
    const {dispatch, fundAccount: {tab2 = {}, exportIsLoad}} = this.props;
      ...
      <div className={cn('tab', 'tab1')}>
        <div className={cn('pcr-condition')}>
          ...
          <Button
            ...
            htmlType={'submit'}
            onClick={() => {
              dispatch({type: 'fundAccount/setTabCondition', payload: {pageIndex: 1}, index: tabIndex});
              this.onSearch();
            }}
          >查询</Button>
            ...
            <div className={cn('advanced_inquire', 'clearfix')}>
              <div className={'search-item'}>
                ...
                <Select
                  ...
                  allowClear
                  onChange={(value) => {
                    dispatch({type: 'fundAccount/setTabCondition', payload: {channelType: value}, index: tabIndex});
                  }}
              ...
              <div className={'search-item'}>
                ...
                <Select
                  ...
                  onChange={(value) => {
                    dispatch({type: 'fundAccount/setTabCondition', payload: {financialType: value}, index: tabIndex});
                  }}
                >
                  {
                    [
              ...
              <div className={'search-item'}>
                ...
                <Select
                  ...
                  onChange={(value) => {
                    dispatch({type: 'fundAccount/setTabCondition', payload: {status: value}, index: tabIndex});
                  }}
                >
                  {
                    [
        ...
        <div className={cn('record-info')}>
          <p>
            <span className={cn('record-1')}>{tab2.statistical.total}条充值记录，</span>
            <span className={cn('record-2')}>充值成功{tab2.statistical.success}笔，</span>
            <span className={cn('record-3')}>审核中{tab2.statistical.underreview}笔，</span>
            <span className={cn('record-4')}>审核不通过{tab2.statistical.failure}笔，</span>
            <span className={cn('record-5')}>待付款{tab2.statistical.pendingpayment}笔</span>
          </p>
          <p>
            <span>现金账户：累计充值 {Tools.getViewPrice(tab2.statisticalAmount.totalamount)} 元，实际到账{Tools.getViewPrice(tab2.statisticalAmount.actualArrival)}元</span>
          </p>
        </div>
            ...
            <div className={cn('footer-pagination')}>
              <Pagination
                ...
                onShowSizeChange={(current, pageSize) => {
                  dispatch({type: 'fundAccount/setTabCondition', payload: {pageSize, pageIndex: 1}, index: tabIndex});
                  this.onSearch();
                }}
                showQuickJumper
                onChange={(pageIndex) => {
                  dispatch({type: 'fundAccount/setTabCondition', payload: {pageIndex}, index: tabIndex});
                  this.onSearch();
                }}
